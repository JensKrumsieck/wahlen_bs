...
dates = [
    date
    for date in dates
    if datetime.datetime.strptime(date["date"], "%d.%m.%Y").year >= 2018
    and not "meister" in date["name"]
    or date["name"].startswith("Europawahl")
    and datetime.datetime.strptime(date["date"], "%d.%m.%Y").year >= 2000
    or date["name"].startswith("Kommunalwahlen")
    and datetime.datetime.strptime(date["date"], "%d.%m.%Y").year
    != 2016  # 2016 not available
    or datetime.datetime.strptime(date["date"], "%d.%m.%Y").year < 2011
    and datetime.datetime.strptime(date["date"], "%d.%m.%Y").year >= 2000
]
...
urls = [
    {
        "name": date["name"],
        "url": urljoin(base_url, date["url"]),
        "date": datetime.datetime.strptime(date["date"], "%d.%m.%Y").year,
    }
    for date in dates
]
...
open_data = [
    {
        "name": (
            "Bundestagswahl"
            if url["name"].startswith("Wahl") and url["date"] == 2009
            else (
                "Landtagswahl"
                if url["name"].startswith("Wahl") and url["date"] == 2008
                else url["name"].split(" ")[0]
            )
        ),
        "date": url["date"],
        "url": (
            urljoin(url["url"], open_data_url)
            if url["name"] != "Landtagswahl"
...
for item in open_data:
    ...
    res = db.execute(
        "INSERT INTO election (name, date) VALUES (?, ?)", (item["name"], item["date"])
    )
    db.commit()
    election_id = res.lastrowid

    # insert parties and get id
    entry = next(f for f in data["dateifelder"] if "meister" not in f["name"])
    parties = entry["parteien"]

    party_mapping = {}
    
    for party in parties:
        party_short = dict_parties.get(party["wert"], party["wert"])
        res = db.execute("SELECT id FROM party WHERE id LIKE ?", (party_short,))
        row = res.fetchone()
        if not row:
            cursor = db.cursor()
    ...
    def add_votes(
        row, election_id, region_id, party_mapping, primary_vote, max_votes, valid_votes
    ):
        # add turnout / weird values for Kommunalwahl
        cursor = db.cursor()
        cursor.execute(
            "INSERT INTO turnout (election_id, region_id, eligible, voted, primary_vote) VALUES (?, ?, ?, ?, ?)",
        ...
        for column_id, party_id in party_mapping.items():
            if primary_vote:
                ...
                columnname = (
                    "D" + column_id + "_summe_liste_kandidaten"
                    if item["name"] == "Kommunalwahlen"
                    else "D" + column_id
                )
            votes = row[columnname]
            if votes and votes > 0:
                cursor.execute(
                    "INSERT INTO vote (election_id, region_id, party_id, votes, primary_vote) VALUES (?, ?, ?, ?, ?)",
                    (election_id, region_id, party_id, votes, primary_vote),
                )
        db.commit()
    ...
    for _, row in df.iterrows():
        ...
        if item["name"] in [
            "Bundestagswahl",
            "Landtagswahl",
        ]:  # erststimme auf D, zweitstimme auf F!
            add_votes(
                row, election_id, region_id, party_mapping, True, max_votes, valid_votes
            )
        else:
            add_votes(
                row,
...
for election_selector in elections:
    ...
    cursor = db.cursor()
    cursor.execute("INSERT INTO election (name, date) VALUES (?, ?)", (name, year))
    db.commit()
    ...
    districts = [83, 3, 4, 5, 69, 82, 71, 7, 8, 72, 76, 77, 78, 79, 80, 81, 73, 74, 75]
    votes = [1, 2]
    ...
    for vote in votes:
        ...
        for district in districts:
            ...
            try:
                region = (
                    soup.find("div", {"class": "ergebnisTabKopf"})
                    .find("table")
                    .find_all("tr")[2]
                    .find_all("td")[1]
                    .text.strip()
                ...
                region = re.search(r"\d{3} [\w-]*", str(region), re.MULTILINE).group(0)
            ...
            max_voters = int(details[1].text.split(":")[1].strip().replace(".", ""))
            valid_votes = int(details[3].text.split(":")[1].strip().replace(".", ""))
            ...
            for i in range(len(results)):
                ...
                item_votes = int(result.find_all("td")[1].text.strip().replace(".", ""))
                election_votes[party] = item_votes
            rows.append(
                {
                    "gebiet": region,
                    "gebiet-nr": region_num,
                    "primary_vote": vote == 1,
        ...
        for _, row in df.iterrows():
            ...
            for party, votes in row.items():
                if party in [
                    ...
                    continue
                cursor.execute("SELECT id FROM party WHERE id LIKE ?", (party,))
                pt = cursor.fetchone()
                if not pt:
                    cursor.execute(
