...
SELECT 
    ...
    v.votes AS votes, 
    v.primary_vote AS vote_primary,
    t.eligible AS eligible_voters, 
    t.voted AS voted_voters, 
    t.primary_vote AS turnout_primary_vote
FROM 
    election e
JOIN 
    vote v ON e.id = v.election_id
JOIN 
    region r ON v.region_id = r.id
JOIN 
    party p ON v.party_id = p.id
LEFT JOIN 
    turnout t ON e.id = t.election_id AND r.id = t.region_id AND v.primary_vote = t.primary_vote
ORDER BY 
    e.id, r.id, v.primary_vote;
"""
cursor.execute(query)

rows = cursor.fetchall()

elections = defaultdict(lambda: {
    "election_id": None,
    "election_date": None,
    "election_name": None,
    "regions": []
...
for row in rows:
    ...
    election["election_id"] = row[0]
    election["election_date"] = row[1]
    election["election_name"] = row[2]
    ...
    party_vote = {
        ...
        "abbreviation": row[8],
        "votes": row[9],
        "isPrimary": bool(row[10])
    }
    region["votes"].append(party_vote)
    election["regions"].append(region)

election_list = list(elections.values())

json_data = json.dumps({"elections": election_list}, indent=4)

with open('db/elections.json', 'w') as json_file:
